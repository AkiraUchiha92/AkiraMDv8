...
`.trim()
    conn.tebakkabupaten[id] = [
        await conn.sendButtonImg(m.chat, await (await fetch(data.result.url)).buffer(), caption, 'ᴹᴿ᭄ AkiraOfcོ', 'Bantuan', '.teka', m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkabupaten[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${data.result.title}*`, 'ᴹᴿ᭄ AkiraOfcོ', 'Tebak Kabupaten', '.tebakkabupaten', conn.tebakkabupaten[id][0])
