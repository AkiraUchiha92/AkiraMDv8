...
`.trim()
    conn.tebakkata[id] = [
        await conn.sendButton(m.chat, caption, 'ᴹᴿ᭄ AkiraOfcོ', 'Bantuan', '.teka', m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkata[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'ᴹᴿ᭄ AkiraOfcོ', 'Tebak Kata', '.tebakkata', conn.tebakkata[id][0])
