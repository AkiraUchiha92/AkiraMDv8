...
`.trim()
    conn.tebakkimia[id] = [
        await conn.sendButton(m.chat, caption, 'ᴹᴿ᭄ AkiraOfcོ', 'Bantuan', '.teki', m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkimia[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.unsur}*`, 'ᴹᴿ᭄ AkiraOfcོ', 'Tebak Kimia', '.tebakkimia', conn.tebakkimia[id][0])
