...
`.trim()
    conn.tebakkota[id] = [
        await conn.sendButton(m.chat, caption, 'ᴹᴿ᭄ AkiraOfcོ', 'Bantuan', '.teko', m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkota[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'ᴹᴿ᭄ AkiraOfcོ', 'Tebak Kota', '.tebakkota', conn.tebakkota[id][0])
